== Small Cloud 的服务注册、发现和配置

Small Cloud使用Alibaba开源的Nacos 提供微服务的的动态服务发现、配置管理和服务管理能力。

使用 Small-Cloud Nacos Config，可基于 Spring Cloud 的编程模型快速接入 Nacos 配置管理功能。
...
具体启动方式参考 Small-Cloud Nacos Discovery 小节的 "Nacos Server 启动" 章节。
